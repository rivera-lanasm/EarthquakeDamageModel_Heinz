...
def make_dictionaries():
    ...
    with arcpy.da.SearchCursor(ConstrType_dbf, ["Code", "Code_Descr", "BLDG_CATEG", "BLDG_SUBCA", "BLDG_TYPE"]) as cursor:
        for row in cursor:
            ConstrTyp_dict.update({row[0]: [row[1], row[2], int(row[3]), row[4]]})
            # dict will have keys = Construction Type Code and
            #  values = Bldg Cat, BldgSubCat, BldgType
        row = None
        cursor = None

    ExtWall_dbf = r"C:\Projects\FEMA\EarthquakeModel\tables\lightbox_exteriorwalltype.dbf"
    with arcpy.da.SearchCursor(ExtWall_dbf, ["Code", "Code_Descr", "BLDG_CATEG", "BLDG_SUBCA", "BLDG_TYPE"]) as cursor:
        for row in cursor:
            ExtWall_dict.update({row[0]: [row[1], row[2], int(row[3]), row[4]]})
...
def lookup_construction_type(parcels, dictionary):
    ...
    with arcpy.da.UpdateCursor(parcels, ["CONSTRUCTION_CODE", "BldgCat", "BldgSubCat", "BldgType", "STOP"]) as cursor:
        for row in cursor:
            CONSTR_TYP = row[0]
            if (CONSTR_TYP == ' ') or (CONSTR_TYP == ''):
                continue
            if (CONSTR_TYP is not None) and (len(CONSTR_TYP) > 1):
                BldgCat = dictionary[CONSTR_TYP][1]
                BldgSubCat = dictionary[CONSTR_TYP][2]
                BldgType = dictionary[CONSTR_TYP][3]
                row[1] = BldgCat
                row[2] = BldgSubCat
                row[3] = BldgType
                row[4] = 'Y'
...
def lookup_exteriorwall_type(parcels, dictionary):
    ...
    with arcpy.da.UpdateCursor(parcels, ["CONSTRUCTION_CODE", "EXTERIOR_WALL_TYPE", "BldgCat", "BldgSubCat", "BldgType", "STOP"]) as cursor:
        for row in cursor:
            CONSTR_TYP = row[0]
            EXT_WALLS = row[1]
            ...
            if (CONSTR_TYP is not None) and (len(CONSTR_TYP) > 1):
                ...
                BldgCat = dictionary[EXT_WALLS][1]
                BldgSubCat = dictionary[EXT_WALLS][2]
                BldgType = dictionary[EXT_WALLS][3]
                row[2] = BldgCat
                row[3] = BldgSubCat
                row[4] = BldgType
                row[5] = 'Y'
